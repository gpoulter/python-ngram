...
class NgramTests(unittest.TestCase):
    ...
    def test_logic(self):
        ...
        self.assertEqual(idx.best_matches('afadfwe', 2),
    ...
    def test_set_ops(self):
        ...
        idx2 = NGram(items2)
        # Item removal
        self.assertEqual(sorted(idx1.search('cde').keys()), ["abcde","cdefg"])
        idx1.remove('abcde')
        self.assertEqual(sorted(idx1.search('cde').keys()), ["cdefg"])
        # Set intersection operation
        items1.remove('abcde')
        idx1.intersection_update(idx2)
        self.assertEqual(idx1, items1.intersection(items2))
        self.assertEqual(sorted(idx1.search('lmn').keys()), [])
        self.assertEqual(sorted(idx1.search('ijk').keys()), [])
        self.assertEqual(sorted(idx1.search('def').keys()), ['cdefg'])
